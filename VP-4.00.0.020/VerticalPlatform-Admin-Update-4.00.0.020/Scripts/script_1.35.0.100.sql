...
BEGIN
	...
	SELECT DISTINCT product_id INTO #orphan_productIds
	FROM
	(
		SELECT product_id
		FROM product_to_category
		WHERE category_id IN (SELECT category_id FROM #orphan) AND
		product_id NOT IN
		(
			SELECT product_id
			FROM product_to_category
			WHERE category_id NOT IN (SELECT category_id FROM #orphan)
		)
	) AS orphan_productIds
