...
BEGIN
	...
	'{=context.ReviewType.Name=}  Product Review Title: {=context.ArticleDetail.Article.Title=}  <br/>  Product: {~if context.ArticleDetail.' +
	'GetRelatedProducts().Count() > 0~}\s{=context.ArticleDetail.GetRelatedProducts()[0].Product.Name=}\s{~end~}\s' +
...
BEGIN
	...
	'.GetRelatedProducts()[0].Product.Name=}\s{~end~}\s  <br/>  {~if context.GiftCardId == nil or context.GiftCardId == '' ~}{~else~}Gift Card Number: {=context.GiftCardId=}  <br/>{~end~}  ' +
