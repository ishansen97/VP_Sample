...
IF NOT EXISTS (
  SELECT 1 
  FROM   sys.columns 
  WHERE  object_id = OBJECT_ID(N'[dbo].[article]') 
         AND name = 'exclude_from_search'
)
BEGIN
	ALTER TABLE article
	ADD [exclude_from_search] BIT NOT NULL DEFAULT (0)
END	
...
BEGIN
	...
	SELECT article_id as id, article_type_id, site_id, article_title, article_sub_title, article_summary,date_published,start_date,end_date,published,external_url_id,
		...
		thumbnail_image_code,article_template_id, open_new_window, flag1, flag2, flag3, flag4, search_content_modified, deleted, exclude_from_search
	FROM article
	WHERE article_id = @id 
...
BEGIN
	...
	(
		SELECT ROW_NUMBER() OVER (ORDER BY article_title) AS titleRowNumber
			...
			, featured_identifier, thumbnail_image_code,article_template_id, open_new_window, flag1, flag2, flag3, flag4, search_content_modified, deleted, exclude_from_search
		FROM article
...
BEGIN
	...
	SELECT distinct a.article_id AS id, a.article_type_id, a.site_id, a.article_title, a.article_sub_title, a.article_summary, 
			...
			a.start_date, a.end_date, a.published, a.article_template_id, a.open_new_window, a.flag1, flag2, a.flag3, a.flag4, a.search_content_modified, deleted, exclude_from_search
	FROM article a
...
BEGIN
	...
	SELECT art.article_id as id, art.article_type_id, art.site_id, art.article_title, art.article_sub_title, art.article_summary,art.date_published,art.start_date,art.end_date,art.published,art.external_url_id,
		...
		art.thumbnail_image_code,art.article_template_id, open_new_window, art.flag1, art.flag2, art.flag3, art.flag4, art.search_content_modified, art.deleted, art.exclude_from_search
	FROM  article art
...
BEGIN
	...
	SELECT a.article_id AS id, a.article_type_id, a.site_id, a.article_title, a.article_sub_title, a.article_summary, 
		...
		a.flag1, flag2, a.flag3, a.flag4, a.search_content_modified, a.deleted, a.exclude_from_search
	FROM article a
...
BEGIN
	...
	is_template, article_template_id, open_new_window,[enabled], created, modified, flag1, flag2, flag3, flag4, search_content_modified, deleted, exclude_from_search
	FROM [article]
...
BEGIN
	...
	SELECT TOP (@batchSize) article_id as id, article_type_id, site_id, article_title, article_sub_title, article_summary, date_published, external_url_id,
		[enabled], modified, created, article_short_title, is_article_template, is_external, featured_identifier,
		thumbnail_image_code, article_template_id, open_new_window, start_date, end_date, published, flag1, flag2, flag3, flag4, search_content_modified, deleted, exclude_from_search
	FROM article
...
BEGIN
	...
	WITH articles (row, id, article_type_id, site_id, article_title, article_sub_title, article_summary, article_short_title, is_article_template, 
		...
		article_template_id, open_new_window, enabled, created, modified, flag1, flag2, flag3, flag4, search_content_modified, deleted, exclude_from_search) AS
	(
...
BEGIN
	...
	SELECT TOP(@batchSize) 
			...
			flag3, flag4, search_content_modified, deleted, exclude_from_search
	FROM
	(
		SELECT article_id 
		FROM article 
...
BEGIN
	...
	WHERE article_id IN (
		SELECT article_id
		FROM #articleId a
			INNER JOIN content_to_content ctc  WITH(NOLOCK)
...
BEGIN
	...
	(
		SELECT v.[vendor_id], a.[article_id] AS [id],a.[article_type_id],a.[site_id],a.[article_title],a.[article_sub_title],a.[article_summary],a.[enabled]
			...
			,a.[legacy_content_id],a.[search_content_modified],a.[deleted],a.exclude_from_search, ROW_NUMBER() OVER (ORDER BY a.[article_title]) AS rowNumber
		FROM [article] a
	...
	SELECT [vendor_id], [id], [article_type_id], [site_id], [article_title], [article_sub_title], [article_summary], [enabled]
		...
		, [legacy_content_id], [search_content_modified], [deleted], exclude_from_search
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE dbo.adminArticle_GetProductReviewsByProductId
	@siteId INT,
	@productId INT,
	@startIndex INT,
	@endIndex INT,
...
GRANT EXECUTE ON dbo.adminArticle_GetProductReviewsByProductId TO VpWebApp 
GO
