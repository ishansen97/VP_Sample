IF NOT EXISTS ( SELECT  *
                FROM    sys.columns
                WHERE   Name = N'locked'
                        AND Object_ID = OBJECT_ID(N'campaign') ) 
    BEGIN
        ALTER TABLE campaign ADD locked BIT NOT NULL DEFAULT 0
    END
GO

IF NOT EXISTS ( SELECT  *
                FROM    sys.columns
                WHERE   Name = N'locked_timestamp'
                        AND Object_ID = OBJECT_ID(N'campaign') ) 
    BEGIN
        ALTER TABLE campaign ADD locked_timestamp SMALLDATETIME NULL DEFAULT NULL
    END
GO
...
--
        ...
        SELECT  campaign_id AS id ,
                ...
                campaign_scheduled_date ,
                campaign_deployed_date ,
                special_comments ,
                text_template_id ,
                campaign_rescheduled_date ,
                recipients_capped ,
                locked ,
                locked_timestamp
        FROM    campaign
...
-- ==========================================================================
    BEGIN
        ...
        IF @campaignTypeId > 0 
            BEGIN
                WITH    CampaignList
                          AS ( SELECT   c.campaign_id AS id ,
                                        ROW_NUMBER() OVER ( ORDER BY c.campaign_id ) AS row_id ,
                                        c.site_id ,
                                        c.campaign_type_id ,
                                        c.[name] ,
                                        c.from_name ,
                                        ...
                                        locked_timestamp
                               FROM     campaign c
                                        INNER JOIN campaign_content_data ccd ON ccd.content_id = @contentId
                               WHERE    c.site_id = @siteId
                                        AND c.campaign_type_id = @campaignTypeId
                                        AND c.campaign_id = ccd.campaign_id
                                        AND ccd.enabled = 1
                             )
                    SELECT  id ,
                            site_id ,
    ...
    BEGIN
        ...
        WITH    CampaignList
                  AS ( SELECT   c.campaign_id AS id ,
                                ...
                                locked_timestamp
                       FROM     campaign c
                                INNER JOIN campaign_type_content_group ctcg ON ctcg.campaign_type_content_group_id = @contentGroupId
                       WHERE    c.site_id = @siteId
                                AND c.campaign_type_id = @campaignTypeId
                                AND c.campaign_id IN (
                                SELECT DISTINCT
                                        ccd.campaign_id
                                FROM    campaign_content_data ccd
                                WHERE   ccd.campaign_type_content_group_id = ctcg.campaign_type_content_group_id
                                        AND ccd.enabled = 1 )
                     )
            SELECT  id ,
                    site_id ,
        ...
        IF @filterContentType = 'Article' 
            BEGIN
                INSERT  INTO #TempList
                        ...
                        WHERE   associated_content_id = @contentId
                                AND content_type_id = 4
                                AND associated_content_type_id = 6
                                AND site_id = @siteId
                                AND associated_site_id = @siteId
                                AND enabled = 1
            END
        ELSE 
            IF @filterContentType = 'Product' 
                BEGIN
                    INSERT  INTO #TempList
                            SELECT DISTINCT
                                    product_id ,
            ...
            BEGIN
                WITH    CampaignList
                          AS ( SELECT   c.campaign_id AS id ,
                                        ...
                                        locked_timestamp
                               FROM     campaign c
                                        INNER JOIN #TempList tl ON tl.siteId = @siteId
                                        INNER JOIN campaign_content_data ccd ON ccd.content_id = tl.contentId
                               WHERE    c.site_id = @siteId
                                        AND c.campaign_type_id = @campaignTypeId
                                        AND c.campaign_id = ccd.campaign_id
                                        AND ccd.enabled = 1
        ...
        BEGIN
            WITH    CampaignList
                      AS ( SELECT DISTINCT
                                    ...
                                    locked_timestamp
                           FROM     campaign c
                                    INNER JOIN content_to_content cc ON c.campaign_id = cc.content_id
                                                              AND cc.content_type_id = 18
...
-- ==========================================================================
    BEGIN
        ...
        SELECT TOP ( @selectLimit )
                ...
                campaign.[subject] ,
                campaign.[private] ,
                campaign.html_template_id ,
                campaign.text_template_id ,
                campaign.[status] ,
                campaign.[enabled] ,
                campaign.created ,
                campaign.modified ,
                campaign.notified_status ,
                campaign.sent_email_reminder ,
                campaign.provider_campaign_id ,
                campaign.campaign_scheduled_date ,
                campaign.campaign_deployed_date ,
                campaign.special_comments ,
                campaign_rescheduled_date ,
                recipients_capped ,
                locked ,
                locked_timestamp
        FROM    campaign
        WHERE   site_id = @siteId
                AND campaign.[name] LIKE @value + '%'
                AND ( @isEnabled IS NULL
                      OR campaign.enabled = @isEnabled
                    )
                AND ( @campaignTypeId IS NULL
                      OR campaign.campaign_type_id = @campaignTypeId
                    )
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE dbo.adminBulkEmail_GetCampaignDetail @id INT
AS -- ==========================================================================
-- $Author: naveen $
-- ==========================================================================
    BEGIN
...
--
        ...
        SELECT  campaign_id AS id ,
                ...
                c.locked_timestamp
        FROM    campaign c
                JOIN campaign_type ct ON c.campaign_type_id = ct.campaign_type_id
                JOIN bulk_email_type bt ON ct.bulk_email_type_id = bt.bulk_email_type_id
...
-- ==========================================================================
    BEGIN
        ...
        WITH    temp_campaign ( row, nameRow, typeRow, rowDesc, nameRowDesc, typeRowDesc, campaign_id, site_id, campaign_type_id, [name], from_name, from_email, [subject], [private], html_template_id, text_template_id, [status], [enabled], created, modified, notified_status, sent_email_reminder, provider_campaign_id, campaign_scheduled_date, campaign_deployed_date, special_comments, campaign_rescheduled_date, recipients_capped, locked, locked_timestamp )
                  AS ( SELECT   ROW_NUMBER() OVER ( ORDER BY campaign.campaign_id ) AS row ,
                                ...
                                ROW_NUMBER() OVER ( ORDER BY campaign_type.[name] DESC ) AS typeRowDesc ,
                                campaign.campaign_id AS campaign_Id ,
                                campaign.site_id AS site_id ,
                                ...
                                campaign.campaign_deployed_date ,
                                campaign.special_comments ,
                                campaign.campaign_rescheduled_date ,
                                campaign.recipients_capped ,
                                campaign.locked ,
                                campaign.locked_timestamp
                       FROM     campaign
                                INNER JOIN campaign_type ON campaign.campaign_type_id = campaign_type.campaign_type_id
            ...
            WHERE   ( @sortBy = 'id'
                      AND @sortOrder = 'asc'
                      AND row BETWEEN @startRowIndex AND @endRowIndex
                    )
                    OR ( @sortBy = 'name'
                         AND @sortOrder = 'asc'
                         AND nameRow BETWEEN @startRowIndex AND @endRowIndex
                       )
                    OR ( @sortBy = 'type'
                         AND @sortOrder = 'asc'
                         AND typeRow BETWEEN @startRowIndex AND @endRowIndex
                       )
                    OR ( @sortBy = 'id'
                         AND @sortOrder = 'desc'
                         AND rowDesc BETWEEN @startRowIndex AND @endRowIndex
                       )
                    OR ( @sortBy = 'name'
                         AND @sortOrder = 'desc'
                         AND nameRowDesc BETWEEN @startRowIndex
                                         AND     @endRowIndex
                       )
                    OR ( @sortBy = 'type'
                         AND @sortOrder = 'desc'
                         AND typeRowDesc BETWEEN @startRowIndex
                                         AND     @endRowIndex
                       )
            ORDER BY CASE WHEN @sortBy = 'id'
                               AND @sortOrder = 'asc' THEN row
                          WHEN @sortBy = 'name'
                               AND @sortOrder = 'asc' THEN nameRow
                          WHEN @sortBy = 'type'
                               AND @sortOrder = 'asc' THEN typeRow
                          WHEN @sortBy = 'id'
                               AND @sortOrder = 'desc' THEN rowDesc
                          WHEN @sortBy = 'name'
                               AND @sortOrder = 'desc' THEN nameRowDesc
                          WHEN @sortBy = 'type'
                               AND @sortOrder = 'desc' THEN typeRowDesc
                     END
        ...
        WHERE   site_id = @siteId
                AND ( @campaignTypeId IS NULL
                      OR campaign_type_id = @campaignTypeId
                    )
...
-- ==========================================================================
    BEGIN
        ...
        WITH    temp_campaign ( row, nameRow, deployedRow, rowDesc, nameRowDesc, deployedRowDesc, campaign_id, site_id, campaign_type_id, [name], from_name, from_email, [subject], [private], html_template_id, text_template_id, [status], [enabled], created, modified, notified_status, sent_email_reminder, provider_campaign_id, campaign_scheduled_date, campaign_deployed_date, special_comments, campaign_rescheduled_date, recipients_capped, locked, locked_timestamp )
                  AS ( SELECT   ROW_NUMBER() OVER ( ORDER BY temp_campaign_union.campaign_id ) AS row ,
                                ...
                                temp_campaign_union.campaign_id AS campaign_Id ,
                                temp_campaign_union.site_id AS site_id ,
                                temp_campaign_union.campaign_type_id AS campaign_type_id ,
                                temp_campaign_union.[name] ,
                                temp_campaign_union.from_name ,
                                temp_campaign_union.from_email ,
                                temp_campaign_union.[subject] ,
                                [private] ,
                                html_template_id ,
                                text_template_id ,
                                temp_campaign_union.[status] ,
                                temp_campaign_union.[enabled] ,
                                temp_campaign_union.created ,
                                temp_campaign_union.modified ,
                                notified_status ,
                                sent_email_reminder ,
                                provider_campaign_id ,
                                campaign_scheduled_date ,
                       ...
                       FROM     ( SELECT    temp_campaign.*
                                  FROM      ( ( SELECT  campaign.*
                                                FROM    campaign
                                                        INNER JOIN ( SELECT
                                                              ...
                                                              FROM
                                                              campaign_type
                                                              INNER JOIN bulk_email_type ON bulk_email_type.bulk_email_type_id = campaign_type.bulk_email_type_id
                                                              AND bulk_email_type.bulk_email_type_id IN (
                                                              SELECT
                                                              [value]
                                                              FROM
                                                              Global_Split(@bulkEmailTypeIds,
                                                              ',') )
                                                              ) temp_campaign_type ON campaign.campaign_type_id = temp_campaign_type.campaign_type_id
                                              ) temp_campaign
                                              LEFT OUTER JOIN campaign_recipient ON temp_campaign.campaign_id = campaign_recipient.campaign_id
                                            )
                                  WHERE     ( temp_campaign.[status] = 7
                                              OR temp_campaign.[status] = 10
                                            )
                                            AND ( @isUserSpecific = 0
                                                  AND temp_campaign.[private] = 1
                                  ...
                                  WHERE     ( campaign.[status] = 7
                                              OR campaign.[status] = 10
                                            )
                                            AND ( @isUserSpecific = 0
                                                  AND campaign.[private] = 0
                                                )
                                ) AS temp_campaign_union
                       WHERE    temp_campaign_union.campaign_deployed_date BETWEEN @startDate
                                                              AND
                                                              @endDate
                                AND ( @campaignTypeId IS NULL
                                      OR temp_campaign_union.campaign_type_id = @campaignTypeId
                                    )
                     )
            SELECT  temp_campaign.campaign_id AS id ,
                    site_id ,
                    campaign_type_id ,
                    [name] ,
                    from_name ,
                    ...
                    html_template_id ,
                    text_template_id ,
                    temp_campaign.[status] ,
                    temp_campaign.[enabled] ,
                    temp_campaign.created ,
                    temp_campaign.modified ,
                    notified_status ,
                    sent_email_reminder ,
                    provider_campaign_id ,
                    campaign_scheduled_date ,
            ...
            WHERE   ( ( @sortBy = 0
                      ...
                      OR ( @sortBy = 1
                           AND @sortOrder = 1
                           AND deployedRow BETWEEN @startRowIndex
                                           AND     @endRowIndex
                         )
                      OR ( @sortBy = 0
                           AND @sortOrder = 2
                      ...
                      OR ( @sortBy = 1
                           AND @sortOrder = 2
                           AND deployedRowDesc BETWEEN @startRowIndex
                                               AND     @endRowIndex
                         )
                    )
            ORDER BY CASE WHEN @sortBy = 0
                               AND @sortOrder = 1 THEN row
                          WHEN @sortBy = 2
                               AND @sortOrder = 1 THEN nameRow
                          WHEN @sortBy = 1
                               AND @sortOrder = 1 THEN deployedRow
                          WHEN @sortBy = 0
                               AND @sortOrder = 2 THEN rowDesc
                          WHEN @sortBy = 2
                               AND @sortOrder = 2 THEN nameRowDesc
                          WHEN @sortBy = 1
                               AND @sortOrder = 2 THEN deployedRowDesc
                     END
        ...
        FROM    ( SELECT    temp_campaign.*
                  FROM      ( ( SELECT  campaign.*
                                FROM    campaign
                                        INNER JOIN ( SELECT campaign_type.campaign_type_id
                                                     FROM   campaign_type
                                                            INNER JOIN bulk_email_type ON bulk_email_type.bulk_email_type_id = campaign_type.bulk_email_type_id
                                                              AND bulk_email_type.bulk_email_type_id IN (
                                                              SELECT
        ...
        BEGIN
            INSERT  INTO #TempAssociatedContentList
                    SELECT DISTINCT
                            ...
                            @siteId
                    FROM    content_to_content
                    WHERE   associated_content_id = @vendorId
                            AND content_type_id IN ( 2, 4 )
                            AND associated_content_type_id = 6
                            AND site_id = @siteId
                            AND associated_site_id = @siteId
                            AND enabled = 1
        ...
        BEGIN
            INSERT  INTO #TempCampaigns
                    SELECT DISTINCT
                            ...
                            locked_timestamp
                    FROM    campaign c
                            INNER JOIN #TempAssociatedContentList t ON t.siteId = c.site_id
                            INNER JOIN campaign_content_data ccd ON ccd.content_id = t.contentId
                    WHERE   c.site_id = @siteId
                            AND c.campaign_id = ccd.campaign_id
                            AND ccd.enabled = 1
                            AND ( @startDate IS NULL
                                  OR c.campaign_rescheduled_date BETWEEN @startDate
                                                              AND
                                                              DATEADD(day, 1,
                                                              @endDate)
                                )
        ...
        BEGIN
            WITH    CampaignList
                      AS ( SELECT DISTINCT
                                    ...
                                    special_comments ,
                                    campaign_rescheduled_date ,
                                    ROW_NUMBER() OVER ( ORDER BY campaign_rescheduled_date DESC ) AS row_id ,
                                    recipients_capped ,
                                    locked ,
                                    locked_timestamp
                           FROM     #TempCampaigns
...
-- ==========================================================================
    BEGIN
        ...
        WITH    temp_campaign ( row, rowDesc, modifiedRow, modifiedRowDesc, idRow, idRowDesc, nameRow, nameRowDesc, statusRow, statusRowDesc, scheduledDateRow, scheduledDateRowDesc, campaignNameRow, campaignNameRowDesc, campaign_id, site_id, campaign_type_id, [name], from_name, from_email, [subject], [private], html_template_id, text_template_id, [status], [enabled], created, modified, notified_status, sent_email_reminder, provider_campaign_id, campaign_scheduled_date, campaign_deployed_date, special_comments, campaign_rescheduled_date, recipients_capped, locked, locked_timestamp )
                  AS ( SELECT   ROW_NUMBER() OVER ( ORDER BY campaign.created ) AS row ,
                       ...
                       FROM     campaign
                                INNER JOIN campaign_type ON campaign.campaign_type_id = campaign_type.campaign_type_id
                       WHERE    campaign.site_id = @siteId
                                AND ( @campaignStatusList IS NULL
                                      OR @campaignStatusList = ''
                                      OR [status] IN (
                                      SELECT    [value]
                                ...
                                AND ( @campaignId = 0
                                      OR campaign.campaign_id = @campaignId
                                    )
                                AND ( @campaignName = ''
                                      OR campaign.[name] LIKE '%'
                                      + @campaignName + '%'
                                    )
                                AND ( @campaignTypeId IS NULL
                                      OR campaign.campaign_type_id = @campaignTypeId
                                    )
                                AND YEAR(campaign.campaign_rescheduled_date) = CASE
                                                              WHEN @campaignYear IS NULL
                                                              THEN YEAR(campaign.campaign_rescheduled_date)
                                                              ELSE @campaignYear
                                                              END
                                AND ( @isPrivate IS NULL
                                      OR campaign.[private] = @isPrivate
                                    )
                     )
            SELECT  campaign_id AS id ,
                    site_id ,
                    ...
                    locked_timestamp
            FROM    temp_campaign
            WHERE   ( @sortBy = 'created'
                      AND @sortOrder = 'asc'
                      AND row BETWEEN @startIndex AND @endIndex
                    )
                    OR ( @sortBy = 'created'
                         AND @sortOrder = 'desc'
                         AND rowDesc BETWEEN @startIndex AND @endIndex
                       )
                    OR ( @sortBy = 'modified'
                         AND @sortOrder = 'asc'
                         AND modifiedRow BETWEEN @startIndex AND @endIndex
                       )
                    OR ( @sortBy = 'modified'
                         AND @sortOrder = 'desc'
                         AND modifiedRowDesc BETWEEN @startIndex AND @endIndex
                       )
                    OR ( @sortBy = 'id'
                         AND @sortOrder = 'asc'
                         AND idRow BETWEEN @startIndex AND @endIndex
                       )
                    OR ( @sortBy = 'id'
                         AND @sortOrder = 'desc'
                         AND idRowDesc BETWEEN @startIndex AND @endIndex
                       )
                    OR ( @sortBy = 'name'
                         AND @sortOrder = 'asc'
                         AND nameRow BETWEEN @startIndex AND @endIndex
                       )
                    OR ( @sortBy = 'name'
                         AND @sortOrder = 'desc'
                         AND nameRowDesc BETWEEN @startIndex AND @endIndex
                       )
                    OR ( @sortBy = 'status'
                         AND @sortOrder = 'asc'
                         AND statusRow BETWEEN @startIndex AND @endIndex
                       )
                    OR ( @sortBy = 'status'
                         AND @sortOrder = 'desc'
                         AND statusRowDesc BETWEEN @startIndex AND @endIndex
                       )
                    OR ( @sortBy = 'schedule'
                         AND @sortOrder = 'asc'
                         AND scheduledDateRow BETWEEN @startIndex
                                              AND     @endIndex
                       )
                    OR ( @sortBy = 'schedule'
                         AND @sortOrder = 'desc'
                         AND scheduledDateRowDesc BETWEEN @startIndex
                                                  AND     @endIndex
                       )
                    OR ( @sortBy = 'campaignType'
                         AND @sortOrder = 'asc'
                         AND campaignNameRow BETWEEN @startIndex AND @endIndex
                       )
                    OR ( @sortBy = 'campaignType'
                         AND @sortOrder = 'desc'
                         AND campaignNameRowDesc BETWEEN @startIndex
                                                 AND     @endIndex
                       )
            ORDER BY CASE WHEN @sortBy = 'created'
                               AND @sortOrder = 'asc' THEN row
                          WHEN @sortBy = 'created'
                               AND @sortOrder = 'desc' THEN rowDesc
                          WHEN @sortBy = 'modified'
                               AND @sortOrder = 'asc' THEN modifiedRow
                          WHEN @sortBy = 'modified'
                               AND @sortOrder = 'desc' THEN modifiedRowDesc
                          WHEN @sortBy = 'id'
                               AND @sortOrder = 'asc' THEN idRow
                          WHEN @sortBy = 'id'
                               AND @sortOrder = 'desc' THEN idRowDesc
                          WHEN @sortBy = 'name'
                               AND @sortOrder = 'asc' THEN nameRow
                          WHEN @sortBy = 'name'
                               AND @sortOrder = 'desc' THEN nameRowDesc
                          WHEN @sortBy = 'status'
                               AND @sortOrder = 'asc' THEN statusRow
                          WHEN @sortBy = 'status'
                               AND @sortOrder = 'desc' THEN statusRowDesc
                          WHEN @sortBy = 'schedule'
                               AND @sortOrder = 'asc' THEN scheduledDateRow
                          WHEN @sortBy = 'schedule'
                               AND @sortOrder = 'desc'
                          THEN scheduledDateRowDesc
                          WHEN @sortBy = 'campaignType'
                               AND @sortOrder = 'asc' THEN campaignNameRow
                          WHEN @sortBy = 'campaignType'
                               AND @sortOrder = 'desc'
                          THEN campaignNameRowDesc
                     END	
        ...
        WHERE   campaign.site_id = @siteId
                ...
                AND ( @campaignName = ''
                      OR campaign.[name] LIKE '%' + @campaignName + '%'
                    )
                AND ( @campaignTypeId IS NULL
                      OR campaign.campaign_type_id = @campaignTypeId
                    )
...
-- ==========================================================================
    BEGIN
        ...
        WITH    temp_campaign ( row, campaign_id, site_id, campaign_type_id, [name], from_name, from_email, [subject], [private], html_template_id, text_template_id, [status], [enabled], created, modified, notified_status, sent_email_reminder, provider_campaign_id, campaign_scheduled_date, campaign_deployed_date, special_comments, campaign_rescheduled_date, recipients_capped, locked, locked_timestamp )
                  AS ( SELECT   ROW_NUMBER() OVER ( ORDER BY campaign_id ) AS row ,
                                campaign_id ,
                                site_id ,
                                campaign_type_id ,
                                [name] ,
    ...
    BEGIN
        ...
        INSERT  INTO #TempCampaigns
                SELECT DISTINCT
                        ...
                        locked_timestamp
                FROM    campaign c
                        INNER JOIN #TempAssociatedContentList t ON t.siteId = @siteId
                        INNER JOIN campaign_content_data ccd ON ccd.content_id = t.contentId
                WHERE   c.site_id = @siteId
                        AND c.campaign_id = ccd.campaign_id
                        AND ccd.enabled = 1
        ...
        BEGIN
            WITH    CountCampaignList
                      AS ( SELECT DISTINCT
                                    ( c.campaign_id )
                           FROM     campaign c
                                    INNER JOIN #TempAssociatedContentList tl ON tl.siteId = @siteId
                                    INNER JOIN campaign_content_data ccd ON ccd.content_id = tl.contentId
                           WHERE    c.site_id = @siteId
                                    AND c.campaign_id = ccd.campaign_id
                                    AND ccd.enabled = 1
...
-- ==========================================================================
    BEGIN
        ...
        VALUES  ( @siteId ,
                  ...
                  @campaignDeployedDate ,
                  @specialComments ,
                  @campaignRescheduledDate ,
                  @recipientsCapped ,
                  @locked ,
                  @lockedTimestamp
                )
...
-- ==========================================================================
    BEGIN
        ...
        SELECT TOP ( @batchSize )
                scs.search_content_status_id AS id ,
                scs.site_id ,
                scs.content_type_id ,
                scs.content_id ,
                scs.[enabled] ,
        ...
        FROM    search_content_status scs
                INNER JOIN product p ON scs.content_id = p.product_id
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE dbo.adminReviewType_GetReviewTypesWithForms @siteId INT
AS -- ==========================================================================
-- $Author: Shehan
-- ==========================================================================
    BEGIN
	
        SELECT  rt.review_type_id AS id ,
                rt.has_image ,
                rt.article_type_id ,
                rt.name ,
                rt.title ,
                ...
                rt.sort_order
        FROM    review_type rt
                INNER JOIN form fm ON fm.content_id = rt.review_type_id
        WHERE   fm.site_id = @siteId
                AND fm.content_type_id = 37
...
UPDATE TOP ( 1 )
        dbo.module
SET     is_container = 1, usercontrol_name = '~/Containers/AccordionContainer/AccordionContainer.ascx'
WHERE   module_name = 'AccordionContainer'
